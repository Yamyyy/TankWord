...
//    private GameType type;// 游戏模式
    private GameStatus gameStatus = GameStatus.gameStatus;
...
//            backgroud = ImageIO.read(new File(ImageUtil.LOGIN_BACKGROUD_IMAGE_URL));// 读取背景图片
            tank = ImageIO.read(ImageSet.class.getClassLoader().getResourceAsStream("resource/image/format/player1.png"));// 读取坦克图标
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    public void paint(Graphics g) {
        ...
        g.drawString("游戏设置", 300, 420);// 绘制第三行文字
    ...
    private void gotoLevelPanel() {
        ...
        frame.setPanel(new TankPanel(gameStatus));// 主窗体跳转至关卡面板
    }
        ...
        switch (code) {// 判断按键值
            ...
            case KeyEvent.VK_ENTER:// 如果按下的是“Enter”
                if (tankY == y1) {// 如果坦克图标在第一个位置
                    gameStatus.setPlayerNumber(1);
                    gotoLevelPanel();// 跳转关卡面板
                }
                if(tankY == y2){
                    gameStatus.setPlayerNumber(2);// 游戏模式为双人模式
                    gotoLevelPanel();// 跳转关卡面板
                }
                if(tankY == y3){     //游戏设置界面

                    frame.removeKeyListener(this);//这里一定要把键盘监听移除掉，否则会自动触发
                    frame.setPanel(new GameSettingPanel(frame));
                }
